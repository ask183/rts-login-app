import React from 'react';
// import DTBImage from '../../assets/downtownbuilding.jpg';
import './SpaceComponent.css';

const DTBImage = require("../../assets/downtownbuilding.jpg");

interface SpaceComponentProperties {
  spaceId: string
  name: string,
  location: string,
  photoUrl?: string,
  reserveSpace: (spaceId: string) => void
}

export class SpaceComponent extends React.Component<SpaceComponentProperties>{

  private renderImage() {
    if (this.props.photoUrl){
      return <img src={this.props.photoUrl} alt=''/>
    } else {
      return <img src={DTBImage} alt='' />
    }
  }

  render() {
    return (<div className='spaceComponent'>
        {this.renderImage()}
        <label className='name'>{this.props.name}</label> <br />
        <label className='spaceId'>{this.props.spaceId}</label> <br />
        <label className='location'>{this.props.location}</label> <br />
        <button onClick={() => this.props.reserveSpace(this.props.spaceId)}>Reserve</button>
      </div>
    )
  }
  
}